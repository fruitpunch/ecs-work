...
void Funix :: chmod(char p[80], char name1[80], char extra[80])
{
  ...
  if(Permission::validPermission(p))//if enter valid mask value
  {
    //change the file/directory mask vaule to new one
    (*currentDir).changePermission(name1, user,777 - atoi(p));
  }//end if
}//end chmod()
...
void Funix :: chown(char newOwner[80], char dirName[80], char moreDir[80])
{
  ...
  if(strcmp(user,"root")!=0){
    ...
    if(strlen(moreDir)>0) (*currentDir).changeOwner(newOwner, moreDir);
  }
}
  ...
  {  
    ...
    {  
      ...
      case 3:
        ...
        break;
      case 4:
	Permission::setUmask(command2, command3);// umask
        break;
      case 5:
