...
Funix :: Funix() : currentDir(new Directory(NULL, NULL, NULL))
{
  ...
  //if directories.txt is empty, nothing will be create
    ...
    infile >> Permissions::umask;// read in the default umask value
  }//end if
  else
  {
    strcpy(user,"root");
    strcpy((*currentDir).File::getPermissions().owner,user);
    Permissions :: umask = -1;//set static variable umask to 22
  }//end else
...
Funix :: ~Funix()
{
  ...
  {
    delete (*currentDir).File::get();// delete the memory of current directory
  }//end if
...
{
  out << fn.user << endl;
  out << *((*fn.currentDir).File::get());//calls directory overloaded operator
  out<<Permissions::umask;
  out<<endl;
  return out;
}//end << operator
...
void Funix :: writePrompt()
{
  ...
  while(1)
  {
    strcpy(path[i],(*temp).File::showName());
...
{   
  ...
  {
    (*currentDir).File::getTime().updateTime();
    //create new directory
    new Directory(currentDir, name1, user);
  }//end if
}//end mkdir

void Funix :: chmod(char p[80], char name1[80], char name2[80])
{
  if(strlen(p) == 0 || strlen(name1) == 0)
  {   
  ...
  if(Permissions :: validPermissions(p))//if enter valid mask value
  {
    ...
    if(strlen(name2)>0)
      (*currentDir).changePermissions(name2, user, 777-atoi(p));
  }//end if
}//end chmod()
...
void Funix :: touch(char name1[80], char name2[80], char name3[80]){
  if(strlen(name1)==0){
    cout<<"touch: too few arguements"<<endl;
    return ;
  }
  (*currentDir).touch(name1, user);

  if(strlen(name2)>0) (*currentDir).touch(name2, user);
  if(strlen(name3)>0) (*currentDir).touch(name3, user);
...
int Funix :: processCommand(char command1[80])
{
  ...
  char const *commands[] = {"cd", "exit", "ls", "mkdir",
                            "umask","chmod","cp","su","chown", "touch", "unknownInput"};
  for(i = 0; i <= 9; i++)
  {
...
void Funix :: run()
{
  ...
  {  
	...
	break;
      case 9:
	touch(command2, command3, command4);
	break;
