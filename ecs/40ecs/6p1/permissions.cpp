...
ofstream& Permissions :: write(ofstream &out)
{
  out<<Permissions::umask;
  return out;
}
ifstream& Permissions :: read(ifstream &in)
{
  int i;
  in >> i;
  Permissions :: umask = i;
...
void Permissions :: printPermissions()
{
  const char* const octal[] = {"rwx", "rw-", "r-x", "r--", "-wx", "-w-",
                               "--x", "---"};// premission list
  cout <<( (t==1)?"d":"-") <<octal[o] << octal[g] << octal[u] << " "<<owner;
}//end printUmask()

void Permissions :: update()
{
  u = perm % 10;//user
  g = ((perm - u) / 10) % 10;// group
  o = ((perm - (g * 10) - u) / 100) % 10;// owner
}//end get()
